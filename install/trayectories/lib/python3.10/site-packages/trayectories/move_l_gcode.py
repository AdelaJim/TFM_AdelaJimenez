...
#
# * trayectoria_dato: Es el nombre del fichero gcode que contiene los puntos de la trayectoria a trazar. Debe estar obligatoriamente dentro de la carpeta ./data/
...
# Clase principal responsable de instanciar a las dos primeras y leer los datos del gcode.
class TrayectoryNodeL(Node):
    ...
    def __init__(self):
        ...
        # Lectura de la trayectoria dato desde el gcode.
        ...
        self.positions, self.Vel_impresion, self.Temp_cama, Temp_extrusor, Motores_on = parseo_gcode(file_path)
        #print(f"Las posiciones son: {self.positions}")

        self.factor_escala=self.get_parameter('factor_escala').value
        self.get_logger().info(f'Vel_impresión: {self.Vel_impresion}')
        #self.get_logger().info(f'Vel_impresión: {self.Vel_impresion[1]}')
